...
export class Triangle extends InteractiveObject {
    ...
    protected checkBounce():void {
        this.speedX = (this._SVGTrinagle.getBBox().x +  this._SVGTrinagle.getBBox().width  > this.canvas.clientWidth)   ?  - Math.abs(this.speedX) : (this._SVGTrinagle.getBBox().x < 0) ? Math.abs(this.speedX) : this.speedX  ;
        this.speedY = (this._SVGTrinagle.getBBox().y +  this._SVGTrinagle.getBBox().height  > this.canvas.clientHeight) ?  - Math.abs(this.speedY) : (this._SVGTrinagle.getBBox().y < 0) ? Math.abs(this.speedY) : this.speedY  ;
    }
